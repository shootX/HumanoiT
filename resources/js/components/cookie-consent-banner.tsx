...
export default function CookieConsentBanner() {
  ...
  useEffect(() => {
    ...
    const isSaas = (props as any).is_saas;

    // Demo mode
    if (isDemo) {
      const authUser = (props as any).auth?.user;
      const isDashboard = window.location.pathname.includes('/dashboard');
      const isBudgetDashboard = window.location.pathname.includes('/budget');
      
      if (!enableLogging || !isDashboard) {
        setIsVisible(false);
        return;
      }
      const dismissed = sessionStorage.getItem('cookie-banner-dismissed');
      const isMainCompanyOwner = authUser?.type === 'company' && 
                                !authUser?.workspace_id && 
                                (!authUser?.created_by || authUser?.created_by === null || authUser?.created_by === 1) &&
                                (authUser?.company_id === authUser?.id || !authUser?.company_id);
      const currentWorkspace = authUser?.current_workspace;
      const currentWorkspaceRole = authUser?.workspaces?.find(w => w.id === currentWorkspace?.id)?.pivot?.role;
      const isOwner = currentWorkspaceRole === 'owner';
      const shouldShow = (isMainCompanyOwner && isOwner) && !isBudgetDashboard && !dismissed;
      setIsVisible(shouldShow);
      return;
    }

    // Main version (SaaS and Non-SaaS)
    if (!enableLogging) {
      setIsVisible(false);
      return;
    }
          ...
          <Card className="w-full max-w-md max-h-[80vh] overflow-y-auto bg-white">
            <div className="p-6">
              <div className="flex items-center mb-4">
                <h3 className="text-lg font-semibold flex-1">{settings.cookieTitle || t('Cookie Preferences')}</h3>
                <Button
                  variant="ghost"
                  size="sm"
