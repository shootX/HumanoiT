...
import { Plus, Search, Video, Calendar, Clock, Users, ExternalLink, Edit, Trash2, Filter, Eye, Copy } from 'lucide-react';
...
export default function GoogleMeetingIndex() {
    ...
    const handleResetFilters = () => {
        ...
        router.get(route('google-meetings.index'), { page: 1 }, { preserveState: false, preserveScroll: false });
    };
    ...
    if (!hasGoogleMeetConfig && !is_demo) {
        ...
        return (
            <PageTemplate title={t('Google Meetings')} url="/google-meetings">
                <Card>
                    <CardContent className="p-6 text-center">
                        ...
                        <p className="text-gray-500 mb-4">
                            {canConfigureGoogleMeet 
                                ? 'Please configure your Google Meet API credentials in settings to use this feature.'
                                : 'Google Meet integration has not been configured for this workspace. Please contact your workspace owner to set up Google Meet credentials.'
                            }
                        </p>
                        {canConfigureGoogleMeet && (
            ...
            <div className="bg-white rounded-lg shadow mb-4 p-4">
                <SearchAndFilterBar
                    ...
                    onPerPageChange={(value) => {
                        ...
                        if (selectedStatus !== 'all') params.status = selectedStatus;
                        if (selectedProject !== 'all') params.project_id = selectedProject;
                        router.get(route('google-meetings.index'), params, { preserveState: false, preserveScroll: true });
                    }}
                            ...
                            {meetings?.data?.length > 0 ? meetings.data.map((meeting: any) => (
                                        ...
                                        <div className="flex gap-1">
                                            {hasPermission(auth?.permissions, 'google_meeting_view') && (
                                                <Button
                                                    variant="ghost"
                                                    size="icon"
                                                    onClick={() => router.get(route('google-meetings.show', meeting.id))}
                                ...
                                <tr>
                                    <td colSpan={7} className="px-6 py-12 text-center">
                                        ...
                                        <p className="text-gray-500 mb-4">
                                            Get started by creating your first Google Meet meeting.
                                        </p>
