...
export function Sydney({ invoice, color, showQr, invoiceUrl, footerTitle, footerNotes, remainingAmount, formatAmount, t }: any) {
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
        <div className="p-8 space-y-8">
          <div className="pb-8 grid grid-cols-3 gap-8" style={{ borderBottom: `3px solid ${template.primary}` }}>
            <div className="col-span-2">
              <h1 className="text-4xl font-bold mb-3" style={{ color: template.secondary }}>INVOICE</h1>
              <div className="text-sm space-y-1"><p><span className="font-semibold">{translate('Invoice Number')}:</span> {invoiceData.invoice_number}</p></div>
            </div>
          ...
          {showQr && (
            <div className="text-center">
              <div className="p-4 rounded-lg border-2 border-dashed flex flex-col items-center mx-auto" style={{ borderColor: `${template.primary}40`, backgroundColor: `${template.primary}05`, width: 'fit-content' }}>
            ...
            <table className="w-full border-collapse rounded-lg overflow-hidden shadow-sm">
              <thead>
                <tr style={{ background: template.primary }}>
                  <th className="text-left py-4 px-6 text-white font-bold text-sm uppercase tracking-wide">{translate('Description')}</th>
                  <th className="text-center py-4 px-6 text-white font-bold text-sm uppercase tracking-wide">{translate('Qty')}</th>
                  <th className="text-right py-4 px-6 text-white font-bold text-sm uppercase tracking-wide">{translate('Unit Price')}</th>
                  <th className="text-right py-4 px-6 text-white font-bold text-sm uppercase tracking-wide">{translate('Total')}</th>
                </tr>
              </thead>
              <tbody className="bg-white">
                {invoiceData.items?.map((item: any, index: number) => (
                  <tr key={item.id} className={`border-b border-gray-200 ${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`}>
                    ...
                    <td className="text-right py-4 px-6 text-gray-700">{displayAmount(item.rate)}</td>
                    <td className="text-right py-4 px-6 font-semibold text-gray-900">{displayAmount(item.amount)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
