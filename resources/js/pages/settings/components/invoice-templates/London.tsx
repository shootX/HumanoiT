...
export function London({ invoice, color, showQr, invoiceUrl, footerTitle, footerNotes, remainingAmount, formatAmount, t }: any) {
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
        <div className="p-8 space-y-8">
          <div className="pb-8" style={{ borderBottom: `3px solid ${template.primary}` }}>
            <div className="flex justify-between items-start">
              {showQr && (
                <div className="text-left">
                  <div className="p-4 rounded-lg border-2 border-dashed flex flex-col items-center" style={{ borderColor: `${template.primary}40`, backgroundColor: `${template.primary}05`, width: 'fit-content' }}>
                    <div className="text-xs text-gray-500 uppercase tracking-wide mb-3">{translate('Scan to Share')}</div>
                    <QRCodeGenerator value={invoiceUrl || 'https://example.com/invoice'} size={100} />
                  </div>
                </div>
              )}
              
              <div className="text-center flex-1">
                <h1 className="text-4xl font-bold mb-3" style={{ color: template.secondary }}>INVOICE</h1>
                <div className="text-sm mb-4"><span className="font-semibold">{translate('Invoice Number')}:</span> {invoiceData.invoice_number}</div>
              </div>
              ...
              <div className="text-right">
                <div className="space-y-1">
                  ...
                  <div className="p-2 rounded" style={{ backgroundColor: `${template.primary}08` }}>
                    ...
                    <div className="font-semibold">{new Date(invoiceData.invoice_date).toLocaleDateString()}</div>
                  </div>
                  <div className="p-2 rounded" style={{ backgroundColor: `${template.primary}08` }}>
                    <div className="text-xs text-gray-500">{translate('Due Date')}</div>
                    <div className="font-semibold">{new Date(invoiceData.due_date).toLocaleDateString()}</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              ...
              <div className="text-sm space-y-1"><p>{invoiceData.client?.name}</p><p>{invoiceData.client?.email}</p></div>
            </div>
            <div>
              <h3 className="text-lg font-bold mb-3" style={{ color: template.primary }}>{translate('PROJECT')}:</h3>
              <div className="p-4 rounded" style={{ backgroundColor: `${template.primary}10` }}><div className="font-bold">{invoiceData.project?.title}</div></div>
            </div>
          </div>
          <table className="w-full border-collapse rounded-lg overflow-hidden shadow-sm">
            <thead><tr style={{ background: template.primary }}>
              <th className="text-left py-4 px-6 text-white font-bold text-sm uppercase">{translate('Description')}</th>
              <th className="text-center py-4 px-6 text-white font-bold text-sm uppercase">{translate('Qty')}</th>
              <th className="text-right py-4 px-6 text-white font-bold text-sm uppercase">{translate('Unit Price')}</th>
              <th className="text-right py-4 px-6 text-white font-bold text-sm uppercase">{translate('Total')}</th>
            </tr></thead>
            <tbody className="bg-white">
              {invoiceData.items?.map((item: any, index: number) => (
                <tr key={item.id} className={`border-b border-gray-200 ${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`}>
                  ...
                  <td className="text-right py-4 px-6">{displayAmount(item.rate)}</td>
                  <td className="text-right py-4 px-6 font-semibold">{displayAmount(item.amount)}</td>
                </tr>
              ))}
            </tbody>
          </table>
