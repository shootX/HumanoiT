...
export default function RecaptchaSettings({ settings = {} }: RecaptchaSettingsProps) {
  ...
  useEffect(() => {
    const src = Object.keys(settings).length > 0 ? settings : (pageProps?.systemSettings || {});
    if (Object.keys(src).length === 0) return;
    setRecaptchaSettings(prev => ({
      ...prev,
      recaptchaEnabled: src.recaptchaEnabled === 'true' || src.recaptchaEnabled === true || src.recaptchaEnabled === 1 || src.recaptchaEnabled === '1',
      recaptchaVersion: src.recaptchaVersion ?? prev.recaptchaVersion,
      recaptchaSiteKey: src.recaptchaSiteKey ?? prev.recaptchaSiteKey,
      recaptchaSecretKey: src.recaptchaSecretKey ?? prev.recaptchaSecretKey,
    }));
  }, [settings, pageProps?.systemSettings]);
