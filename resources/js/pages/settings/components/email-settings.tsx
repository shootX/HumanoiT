...
export default function EmailSettings() {
  ...
  useEffect(() => {
    ...
    setEmailSettings(prev => ({
      ...prev,
      provider: settings.email_provider ?? prev.provider,
      driver: settings.email_driver ?? prev.driver,
      host: settings.email_host ?? prev.host,
      port: settings.email_port ?? prev.port,
      username: settings.email_username ?? prev.username,
      password: settings.email_password ? '••••••••••••' : prev.password,
      encryption: settings.email_encryption ?? prev.encryption,
      fromAddress: settings.email_from_address ?? prev.fromAddress,
      fromName: settings.email_from_name ?? prev.fromName,
    }));
  }, [settings]);

  // Test email = user's own email only (abuse prevention)
  ...
  const sendTestEmail = (e: React.FormEvent) => {
    ...
    router.post(route('settings.email.test'), { email: userEmail }, {
      preserveScroll: true,
      onSuccess: (page) => {
        setIsSending(false);
        toast.dismiss();
        ...
        if (successMessage) {
          ...
          const message = t('Test email sent successfully to {{email}}', { email: userEmail });
              ...
              <form onSubmit={sendTestEmail} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("Test email is sent only to your account email (for security)")}
                  </p>
                </div>
