...
export default function CurrencySettings() {
    ...
    // Sync state when page props change (e.g. after save + redirect)
    useEffect(() => {
        const sys = systemSettings || {};
        if (Object.keys(sys).length === 0) return;
        setCurrencySettings(prev => ({
            ...prev,
            decimalFormat: sys.decimalFormat ?? prev.decimalFormat,
            defaultCurrency: sys.defaultCurrency ?? prev.defaultCurrency,
            decimalSeparator: sys.decimalSeparator ?? prev.decimalSeparator,
            thousandsSeparator: sys.thousandsSeparator ?? prev.thousandsSeparator,
            floatNumber: sys.floatNumber !== '0',
            currencySymbolSpace: sys.currencySymbolSpace === '1',
            currencySymbolPosition: sys.currencySymbolPosition ?? prev.currencySymbolPosition,
        }));
    }, [systemSettings]);
    ...
    useEffect(() => {
        if (currencies && currencies.length > 0) {
            ...
            if (selectedCurrency) {
                ...
                const fallbackCurrency = currencies.find((c: CurrencyProps) => c.is_default) || currencies[0];
                if (fallbackCurrency) {
                    setCurrencySettings(prev => ({
                        ...prev,
                        defaultCurrency: fallbackCurrency.code,
                        currencyName: fallbackCurrency.name
                    }));
                }
            }
        }
    ...
    const submitCurrencySettings = (e: React.FormEvent) => {
        ...
        router.post(route('settings.currency.update'), payload, {
            preserveScroll: true,
            onSuccess: (page) => {
                toast.dismiss();
