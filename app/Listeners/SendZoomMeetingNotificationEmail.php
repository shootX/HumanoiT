        ...
        if (isEmailTemplateEnabled('Zoom Meeting Notification', createdBy())) {
            foreach ($meeting->members as $member) {
                ...
                try {
                    ...
                    session()->flash('success', 'Zoom Meeting email sent successfully');
                } catch (Exception $e) {
                    \Log::error('Zoom meeting notification email failed: ' . $e->getMessage(), [
                        'meeting_id' => $meeting->id,
                        'member_email' => $member->email
                    ]);
                    session()->flash('error', 'Failed to send Zoom meeting notification email');
                }
            }
        }
    }
