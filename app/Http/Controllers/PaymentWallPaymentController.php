...
class PaymentWallPaymentController extends Controller
{
    ...
    public function createInvoicePayment(Request $request)
    {
        try {
            ...
            if (!isset($settings['paymentwall_public_key']) || !isset($settings['is_paymentwall_enabled']) || $settings['is_paymentwall_enabled'] !== '1') {
                return response()->json(['error' => __('PaymentWall not configured')], 400);
            }

            $isTestMode = ($settings['paymentwall_mode'] ?? 'sandbox') === 'sandbox';

            return response()->json([
                'success' => true,
                'brick_config' => [
                    'public_key' => $settings['paymentwall_public_key'],
                    'amount' => $request->amount,
                    'currency' => 'USD',
                    'description' => 'Invoice Payment - ' . $invoice->invoice_number,
    ...
    public function processInvoicePaymentFromLink(Request $request, $token)
    {
        try {
            ...
            if (!isset($settings['is_paymentwall_enabled']) || $settings['is_paymentwall_enabled'] !== '1') {
                return response()->json(['error' => 'PaymentWall not enabled'], 400);
            }

            if (!isset($settings['paymentwall_public_key'])) {
                return response()->json(['error' => 'PaymentWall not configured'], 400);
            }
    ...
    public function invoiceSuccessFromLink(Request $request, $token)
    {
        try {
            ...
            if ($response === 'success' && $orderId && $amount) {
                $invoice = \App\Models\Invoice::where('payment_token', $token)->firstOrFail();
